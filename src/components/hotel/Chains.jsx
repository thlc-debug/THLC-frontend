...
function Chains() {
  ...
  return (
    <div className='mt-[7rem] max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8'>
      ...
      <div className='text-xl sm:text-2xl text-gray-500'>55+ chains curated for you..</div>

      <div className='p-5 h-auto sm:h-[305px] my-5 w-full bg-gray-100 border-2 border-gray-200 rounded-xl'>
        <Slider {...settings}>
          {hotels.map((item, index) => (
            <Link href={`/hoteldetail?id=${item._id}`} key={index} className='p-5 relative hover:cursor-pointer no-focus-outline block'>
              <div className='relative'>
                ...
                <div className="absolute bottom-0 w-full py-2 bg-black bg-opacity-50 rounded-b-xl text-white text-center">
                  ...
                  <h4 className="text-md font-f_3">{item.city}, {item.country}</h4>
                </div>
              </div>
            </Link>
          ))}
