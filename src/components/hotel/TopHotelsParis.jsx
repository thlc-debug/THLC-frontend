  ...
  render() {
    ...
    return (
      <div className='my-[7rem] w-full max-w-[1200px] mx-auto px-4'>
        <div className='text-2xl md:text-4xl font-f_3'>Top hotels in <b>{city}</b></div>
        <div className='p-4 sm:p-6 md:p-10 h-auto md:h-[380px] my-5 w-full bg-gray-100 border-2 border-gray-200 rounded-xl'>
          <Slider {...settings}>
            {hotels.map(item => (
              <Link href={`/hotelsdetail?id=${item._id}`} className="p-3 relative hover:cursor-pointer no-focus-outline block" key={item._id}>
