...
const Hotels = () => {
  ...
  const fetchCountries = async () => {
    try {
      ...
      const limitedCountries = ['All', ...data.slice(0, 8).map(country => country._id)];
  ...
  return (
        ...
        <div className="flex space-x-4 overflow-x-auto sm:overflow-visible scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-200">
          {countries.map((country) => (
            <button
              ...
              className={`relative px-4 py-2 mb-4 border-2 rounded-lg h-16 overflow-clip transition-transform duration-200 whitespace-nowrap ${selectedCountry === country ? 'font-bold border-gray-800' : 'border-gray-300'
                }`}
            >
              {country}
      ...
      <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredHotels.slice(0, visibleHotels).map((hotel) => (
          <Link href={`/hotelsdetail?id=${hotel._id}`} key={hotel._id}>
            <div key={hotel.id} className="relative bg-white h-[20rem] shadow-lg rounded-lg overflow-hidden group">
              <div className="group-hover:scale-105 group-hover:cursor-pointer transition-transform duration-300">
                <Image
                  src={hotel.photoUrls[0].replace("www.dropbox.com", "dl.dropboxusercontent.com")}
                  alt={hotel.name}
          ...
          <div className="p-4">
            ...
            <p className="text-gray-500 text-md mb-4">{hotel.city}, {hotel.country}</p>
          </div>
        </div>
        </Link>
      ))}
    ...
    {visibleHotels < filteredHotels.length && (
      <div className="flex justify-center mt-6">
        <button onClick={loadMoreHotels} className="px-6 py-2 border-2 border-gray-400 text-black rounded-full hover:bg-gray-100 transition-colors">
          Load more
        </button>
      </div>
    )}
